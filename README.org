...
Parakeet is a local, on-device ASR backend using sherpa-onnx on the
CPU and needs no API key.  After a consent prompt, its model is
downloaded on first use into ~~/.local/share/talk-rs/models/~.
