...
- *Multiple providers* -- Mistral (Voxtral) and OpenAI (Whisper /
  GPT-4o) for batch transcription; OpenAI realtime streaming via
  WebSocket.
...
| ~audio.sample_rate~          | Sample rate in Hz (e.g. ~16000~)                     |
| ~audio.channels~             | Channel count (~1~ = mono)                           |
| ~audio.bitrate~              | Bitrate in bps (e.g. ~32000~)                        |

*** Optional fields

| Field                              | Default                   | Description                        |
|------------------------------------+---------------------------+------------------------------------|
| ~providers.mistral.model~          | ~voxtral-mini-latest~     | Mistral transcription model        |
...
export TALK_RS_AUDIO_SAMPLE_RATE=44100
#+end_src
...
| ~--realtime~      | Stream audio via WebSocket (OpenAI) |
